...
PyObject* BSplineCurvePy::buildFromPoles(PyObject* args)
{
    ...
    try {
        ...
        if (Base::asBoolean(periodic)) {
            ...
            Handle(Geom_BSplineCurve) spline = new Geom_BSplineCurve(poles, knots, mults, degree, true);
            if (!spline.IsNull()) {
                this->getGeomBSplineCurvePtr()->setHandle(spline);
                Py_Return;
            }
            ...
            Handle(Geom_BSplineCurve) spline
                = new Geom_BSplineCurve(poles, knots, mults, degree, false);
            if (!spline.IsNull()) {
                this->getGeomBSplineCurvePtr()->setHandle(spline);
                Py_Return;
            }
...
PyObject* BSplineCurvePy::buildFromPolesMultsKnots(PyObject* args, PyObject* keywds)
{
    ...
    try {
        ...
        for (int i = 2; i < occmults.Length(); i++) {
